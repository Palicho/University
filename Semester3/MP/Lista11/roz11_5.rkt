  ...
  (type-case Exp e
    ...
    [(let*E xs e)
     (type-case (Listof (Symbol * Exp)) xs
       [empty (eval e env)]
       [(cons s ss)
        (let ([v (eval (snd s) env)])
          (eval (let*E ss e) (extend-env env (fst s) v)))])]
    [(letE xs e)
     (eval-let xs e env)]))

(define (eval-let xs [e : Exp] [env : Env]) : Value
  (type-case (Listof (Symbol * Exp)) xs
    [empty (eval e env)]
    [(cons s ss)
     (let ([v (eval-let ss (snd s) env)])
       (eval (letE ss e) (extend-env env (fst s) v)))]))
