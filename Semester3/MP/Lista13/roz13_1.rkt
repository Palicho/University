...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `SYMBOL s)
     (varE (s-exp->symbol s))]
    [(s-exp-match? `{SYMBOL} s)
     (symE (s-exp->symbol (first (s-exp->list s))))]
...
(define init-env 
  (foldr (λ (b env) (extend-env env (fst b) (snd b)))
         ...
         (list (pair '+ (op-num-num->value +))
               ...
               (pair '<= (op-num-bool->value <=))
               (pair 'symbol=? (op-sym-bool->value symbol=?))
               (pair 'symbol? (op-s-bool->value))
               (pair 'number? (op-n-bool->value)))))
