...
(define mtE : StackE empty)

(define (pushE [v : Exp] [s : StackE]) : StackE
  (cons v s))

(define (popE [s : StackE]) : (Exp * StackE)
  (type-case StackE s
    [empty
     (error 'popE "empty stack")]
    [(cons v s)
     (pair v s)]))

(define (decompile [c : Code] [s : StackE]) : Exp
  (type-case Code c
    [empty
     (fst (popE s))]
    [(cons i c)
