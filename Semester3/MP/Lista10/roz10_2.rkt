...
(define (|| [p : Boolean] [ q : Boolean] ) : Boolean
  (or p q))

(define (&& [p : Boolean] [ q : Boolean] ) : Boolean
  (and p q))

(define (! [p : Boolean])  : Boolean
  (not p))

(define (^ [p : Number])  : Number
  (- 0 p))
...
(define (parse-up [up : Symbol]) : Up
  (cond 
    [(eq? up '! ) (notb)]
    [(eq? up '^ ) (neg)]
    [else (error 'parse "unkown u-operator")]))
...
(define (up-bool->proc [f : (Boolean -> Boolean)]) : (Value -> Value)
  (λ (v)
    (type-case Value v
      [(boolV b) (boolV (f b))]
...
(define (up-number->proc [f : (Number -> Number)]) : (Value -> Value)
  (λ (v)
    (type-case Value v
      [(numV b) (numV (f b))]
      [else (error 'eval "type error")])))
...
(define (op->proc [op : Op]) : (Value Value -> Value)
  (type-case Op op
    ...
    [(alt) (op-bool-bool->proc ||)]
    [(conj) (op-bool-bool->proc &&)]))
